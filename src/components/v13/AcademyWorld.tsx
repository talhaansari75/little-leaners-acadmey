...
type AcademyClass = "Nursery" | "KG" | "Montessori";
...
const CLASSES: Record<AcademyClass, { icon: string; color: string; tagline: string; subjects: string[]; rooms: string[]; mascot: string; mission: string }> = {
  ...
  KG: { icon: "🔵", color: "academy-kg", tagline: "Curious school readiness", subjects: ["Phonics & CVC", "Numbers to 100", "Early maths", "Reading & science"], rooms: ["KG Classroom", "Math Mountain", "Reading Library", "Mini Science Lab"], mascot: "🦊", mission: "Build 3 simple words!" },
  Montessori: { icon: "🌱", color: "academy-montessori", tagline: "Calm hands-on discovery", subjects: ["Practical Life", "Sensorial", "Language", "Mathematics & Culture"], rooms: ["Montessori Garden", "Practical Life Table", "Sensorial Shelf", "Nature Corner"], mascot: "🐼", mission: "Sort objects by size!" },
};
const REWARDS: Record<AcademyClass, string[]> = {
  Nursery: ["🏅 ABC Star", "🔢 Counting Star", "🎨 Color Star", "🎵 Rhyme Star"],
  KG: ["🔤 Phonics Star", "🔢 Math Star", "📖 Reading Star", "🔬 Discovery Star"],
  Montessori: ["🌱 Practical Life", "🖐️ Sensorial Star", "🔤 Language Star", "🌿 Nature Explorer"],
};
const QUICK = [
...
export function AcademyWorld({ xp, completed, available, onSpeak, onMissionComplete }: Props) {
  const [klass, setKlass] = useState<AcademyClass>(() => {
    try { return (localStorage.getItem("lla-class") as AcademyClass) || "Nursery"; } catch { return "Nursery"; }
  });
  ...
  return <section className={`academy-world ${data.color} rounded-[2rem] p-4 shadow-lg`}>
    ...
    <div className="relative z-10">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1"><p className="text-[10px] font-black uppercase tracking-[.16em] opacity-70">Welcome to your school</p><h2 className="font-display text-2xl text-slate-800">My Academy 🏫 • Little Learners Academy</h2><p className="text-xs text-slate-600">Three separate pathways • {progress}% journey</p></div>
      ...
      <div className="mt-2 grid grid-cols-2 gap-2">{data.rooms.map((room, i) => <button key={room} type="button" disabled={i >= roomCount} onClick={() => { const ids: Record<AcademyClass,string[]> = { Nursery:["colors","letters","numbers","rhymes"], KG:["kg-phonics","kg-math","kg-reading","kg-vehicles"], Montessori:["mont-sensorial","mont-practical","mont-sorting","mont-nature"] }; openActivity(ids[klass][i]); }} className={`academy-room ${i < roomCount ? "" : "is-locked"}`}><span>{["🌈", "🔤", "🔢", "🎵"][i]}</span><b>{room}</b>{i >= roomCount && <small>🔒 {100 * (i + 1)} XP</small>}</button>)}</div>
      ...
      <div className="academy-bus mt-3"><div className="academy-bus-icon"><Bus className="size-6"/></div><div className="flex-1"><b>Magic School Bus 🚌</b><small>Nursery • KG • Montessori • Art • Music • Stories</small></div><button onClick={() => window.dispatchEvent(new CustomEvent("lla-open-room", { detail: { room: "bus" } }))}>Ride!</button></div>
