...
const CLASS_EMOJI: Record<LearningClass, string> = { Nursery: "🟢", KG: "🔵", Montessori: "🌱" };
...
export function SmartJourneyPanel({ className, onSpeak }: { className?: string; onSpeak: (text: string) => void }) {
  ...
  const changeClass = (next: LearningClass) => { setKlass(next); try { localStorage.setItem("lla-class", next); } catch {} window.dispatchEvent(new Event("lla-class-change")); setTick((v) => v + 1); onSpeak(`${next} smart journey selected.`); };
  return <section className={`smart-journey ${className ?? ""}`} aria-label="Smart Learning Journey">
    ...
    {tab === "adventure" && <div className="smart-adventure"><div className="smart-class-picker">{(["Nursery","KG","Montessori"] as LearningClass[]).map((name) => <button key={name} type="button" onClick={() => changeClass(name)} className={klass === name ? "is-active" : ""}>{CLASS_EMOJI[name]} {name}</button>)}</div><div className="smart-daily"><div className="smart-daily-icon">{recommendation.mode === "audio" ? "🔊" : recommendation.mode === "story" ? "📖" : recommendation.mode === "hands-on" ? "👐" : "🎮"}</div><div className="min-w-0 flex-1"><span>Today’s smart challenge</span><b>{challenge}</b><small>{recommendation.title} • {recommendation.difficulty} • {mode}</small></div><button type="button" onClick={() => { const map: Record<LearningClass, Record<string,string>> = { Nursery: { letters:"letters", numbers:"numbers", colors:"colors", shapes:"shapes", animals:"animals", rhymes:"rhymes" }, KG: { phonics:"kg-phonics", words:"kg-reading", math:"kg-math", reading:"kg-reading", patterns:"kg-patterns", science:"kg-vehicles" }, Montessori: { "practical-life":"mont-practical", sensorial:"mont-sensorial", language:"mont-language", mathematics:"mont-math", sorting:"mont-sorting", nature:"mont-nature" } }; const id=map[klass][recommendation.skill]; if(id) window.dispatchEvent(new CustomEvent("lla-open-activity", { detail: { id, difficulty: recommendation.difficulty } })); onSpeak(`${challenge}. ${recommendation.title}. ${recommendation.reason}`); }}><Gamepad2 className="size-4"/> Start</button></div><div className="smart-mini-stats"><span><Target className="size-3.5"/> {explored}/{skills.length} skills explored</span><span><CheckCircle2 className="size-3.5"/> {recommendation.confidence}% guidance</span></div></div>}
    {tab === "skills" && <div className="smart-skill-map"><div className="smart-class-picker">{(["Nursery","KG","Montessori"] as LearningClass[]).map((name) => <button key={name} type="button" onClick={() => changeClass(name)} className={klass === name ? "is-active" : ""}>{CLASS_EMOJI[name]} {name}</button>)}</div><div className="smart-skill-grid">{skills.map((skill) => { const s = stat[skill]; const accuracy = s?.attempts ? Math.round((s.correct / s.attempts) * 100) : 0; const state = !s?.attempts ? "New" : accuracy >= 88 ? "Strong" : accuracy >= 65 ? "Growing" : "Practice"; return <div key={skill} className={`smart-skill ${state.toLowerCase()}`}><div><b>{skill.replaceAll("-", " ")}</b><span>{state}</span></div><div className="smart-skill-bar"><i style={{ width: `${s?.attempts ? Math.max(8, accuracy) : 6}%` }}/></div></div>; })}</div><p className="smart-note"><Map className="size-4"/> The map uses only this class’s learning signals, so pathways stay separate.</p></div>}
