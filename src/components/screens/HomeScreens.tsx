  ...
  return (
    <div className="app-shell starfield safe-pad flex h-dvh flex-col gap-4 overflow-y-auto">
      ...
      <button type="button" className="journey-continue btn-primary animate-pop" onClick={() => useGame.getState().go("preschool")}>
        <span><span className="block text-xs uppercase tracking-[0.2em] opacity-80">Little Learners Academy</span><strong className="block text-lg">Enter Nursery, KG or Montessori</strong></span><span className="text-2xl">→</span>
      </button>
      <JourneyHero />
      <AdBanner />
      {(() => {
